<script lang="ts">
  import { todos } from "../Stores/TodoStore";
  import TodoList from "../components/TodoList.svelte";

  let inputValue: string = ''

  const addTodo = (e: KeyboardEvent) => {
    // check if the key pressed is enter
    if (e.key !== "Enter") return;
    add(e);
  };
  
  const onclick = (e: MouseEvent) => {
    add(e);
  }
  
  const add = (e:any) =>  {
    if (inputValue == '' || inputValue == ' ') return;
    
    // using the store method to add new Todo
    todos.addTodo(0, inputValue);
    
    // reset the input value
    inputValue = "";
    e.preventDefault();
  }
</script>

<form class="sepration">
  <div class="input-container">
      <input required class="todo-input placeholder-white focus:ring-0" bind:value={inputValue} on:keydown={(e) => {addTodo(e);}} type="text" placeholder="What do you want to do?"/>

      <button on:click={(e) => onclick(e)} class="icon">
        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="2rem" height="2rem" viewBox="0 -2 32 32" version="1.1">
          <title>arrow-right</title>
          <defs></defs>
            <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd" >
                <g id="Icon-Set" transform="translate(-360.000000, -933.000000)" fill="#fff">
                    <path d="M388,933 L368,933 C365.791,933 364,934.791 364,937 L364,941 L366,941 L366,937 C366,935.896 366.896,935 368,935 L388,935 C389.104,935 390,935.896 390,937 L390,957 C390,958.104 389.104,959 388,959 L368,959 C366.896,959 366,958.104 366,957 L366,953 L364,953 L364,957 C364,959.209 365.791,961 368,961 L388,961 C390.209,961 392,959.209 392,957 L392,937 C392,934.791 390.209,933 388,933 L388,933 Z M377.343,953.243 C376.953,953.633 376.953,954.267 377.343,954.657 C377.733,955.048 378.367,955.048 378.758,954.657 L385.657,947.758 C385.865,947.549 385.954,947.272 385.94,947 C385.954,946.728 385.865,946.451 385.657,946.243 L378.758,939.344 C378.367,938.953 377.733,938.953 377.343,939.344 C376.953,939.733 376.953,940.367 377.343,940.758 L382.586,946 L361,946 C360.447,946 360,946.448 360,947 C360,947.553 360.447,948 361,948 L382.586,948 L377.343,953.243 L377.343,953.243 Z" id="arrow-right">
              </path>
              </g>
          </g>
      </svg>
      </button>
  </div>
</form>

<!-- Container for all Todo lists -->
<div class="todo-lists-wrapper sepration">
  <TodoList todoLists={$todos}/>
</div>

<style>
  .todo-input {
    border-radius: 1rem;
    width: 100%;
    font-size: 1.2rem;
    padding: 1.3rem 4.5rem 1.3rem 1.3rem;
    border: 2px solid #727272;
    background-color: #333333;
    color: #fff;
  }

  .input-container {
    display: flex;
    position: relative;
  }

  .todo-lists-wrapper {
    display: grid;
    gap: 2rem;
  }

  @media screen and (min-width: 1023px) {
    .todo-lists-wrapper {
      grid-template-columns: repeat(2, 1fr);
    }
  }

  @media screen and (min-width: 1280px) {
    .todo-lists-wrapper {
      grid-template-columns: repeat(3, 1fr);
    }
  }

  .icon {
    position: absolute;
    RIGHT: 1.5rem;
    top: 50%;
    transform: translateY(-50%);
    cursor: pointer;
    background: none;
    border: none;
  }
</style>
